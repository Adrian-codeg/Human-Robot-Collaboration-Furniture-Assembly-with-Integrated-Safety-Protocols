...
public:
    ...
    void ur5_place(moveit::planning_interface::MoveGroupInterface& group)
    {
        ...
        place_location[0].place_pose.pose.position.x = 0.488; //-0.02
        place_location[0].place_pose.pose.position.y = 0.035-0.045; // +0.01
             ...
             moveit::planning_interface::PlanningSceneInterface& planning_scene_interface)
    {
        ...
        while (ros::ok())
        {
            ...
            {
                ROS_INFO("Waiting for clear path (stop: %d, slow_down: %d)", stop, slow_down);
